...
public class JdbcQueryLookupStrategyUnitTests {
	...
	public void typeBasedRowMapperGetsUsedForQuery() {
		...
		RowMapper stringMapper = mock(RowMapper.class);
		RowMapperMap rowMapperMap = new ConfigurableRowMapperMap().register(String.class, stringMapper);

		RepositoryQuery repositoryQuery = getRepositoryQuery("returningString", rowMapperMap);

		repositoryQuery.execute(new Object[] {});

		verify(mappingContext.getTemplate()).query(anyString(), any(SqlParameterSource.class), eq(stringMapper));
	}
